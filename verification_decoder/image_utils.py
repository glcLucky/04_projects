import numpy as np
import glob
from . import code_utils as cu
from PIL import Image


class ImageUtils():
    def __init__(self):
        self.test_data = glob.glob(r'E:\07_data\00_practise\verification_code\test/*.jpg')
        self.test_label = np.array(
            [cu.in_transition(self.test_data[index].split('\\')[-1].split('.')[0].split('_')[0]) for index in
             range(len(self.test_data))])

        self.train_data = glob.glob(r'E:\07_data\00_practise\verification_code\train/*.jpg')
